...
function ArticleDetail() {
  ...
  if (!data) return <PageShell><div className="container-x py-20 text-center text-muted-foreground">Article not found.</div></PageShell>;

  return (
    <PageShell>
      <article className="container-x py-12 max-w-3xl">
        <Link to="/articles" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground mb-4">
          <ArrowLeft className="h-4 w-4" /> All articles
        </Link>
        {data.column_name && <p className="text-xs font-semibold uppercase tracking-wider text-brand">{data.column_name}</p>}
